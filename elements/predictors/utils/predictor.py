...
class Predictor:
    ...
    def predict(self, image: np.ndarray) -> np.ndarray:
        ...
        if isinstance(self.model_settings.model, ultralytics.models.yolo.model.YOLO):
            predictions = self.model_settings.model(image, imgsz=image.shape[0], verbose=False, conf=self.general_settings.box_threshold, device="0" if "cuda" in self.model_settings.device else "cpu")
        else:
