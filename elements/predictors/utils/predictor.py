...
class Predictor:
    ...
    def __init__(self, model: torch.nn.Module, box_threshold: float, device: str):
        self.model = model
    ...
    def predict(self, image: np.ndarray):
        ...
        if isinstance(self.model, ultralytics.models.yolo.model.YOLO):
            predictions = self.model(image, verbose=True, conf=self.box_threshold, device=self.device)
        else:
