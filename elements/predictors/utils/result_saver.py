...
class ResultSaver:
    ...
    def initiate_result_video(self, width: int, height: int, fps: float):
        """
        Instantiates a cv2.VideoWriter object
        """
        self.out_file = os.path.join(self.output_folder, "output", "videos", f"{time.time()}.mp4")
        ...
        # Video writer
        self.out_video = cv2.VideoWriter(self.out_file, fourcc, fps, self.frame_size)

    def append_image_to_video(self, image: np.ndarray):
        """
    ...
    def save_copy_video(self):
        """
        Copies the fully analyzed video to a local directory
        """
        self.local_cache_copy = os.path.join(self.local_output_folder, "videos", os.path.basename(self.out_file))
        shutil.copy(src=self.out_file, dst=self.local_cache_copy)
        self.logger.info(f"Saving video to: {self.local_cache_copy}")
        return self.local_cache_copy
