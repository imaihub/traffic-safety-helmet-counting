...
class AdvancedViewSetting(ParamSetting):
    ...
    -------------------------------|--------------------------------------------------------------
    Dynamic Processing             | camera_mode == InputMode.FILE
    Box threshold
    ...
    def update(self, advanced_view: bool, **params):
        with self.locker.lock:
            ...
            dynamic_processing_box = gr.Checkbox(label="Dynamic Processing", interactive=True, value=self.general_settings.dynamic_processing,
            ...
            tracker_option_3 = gr.Text(label="Maximum age", value=str(self.tracking_settings.param_options.get(
                self.tracking_settings.current_options[2], "-")), interactive=True, visible=advanced_view and self.general_settings.task_type.casefold() == Tasks.TRACKING.name.casefold() is not None and not self.tracking_settings.current_options[2] == "")
        ...
        return bit_box, width_input_box, height_input_box, box_threshold_box, gamma_correction_bool_box, gamma_value_box, tracker_option_1, tracker_option_2, tracker_option_3, tracker_option_4, dynamic_processing_box
